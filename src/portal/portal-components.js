...
// Merged hider walls – 9 planes → 1 draw call
registerComponent('merged-hider-walls', {
  init() {
    const {THREE} = window
    const mat = getSharedDepthMaterial(false)
    const planes = [
      {w: 28, h: 13.33292, pos: [0, 7.33354, 0.005], rot: [0, 0, 0]},
      {w: 28, h: 13.33292, pos: [0, -7.33354, 0.005], rot: [0, 0, 0]},
      {w: 13.5, h: 1.3341666667, pos: [-7.25, 0, 0.005], rot: [0, 0, 0]},
      {w: 13.5, h: 1.3341666667, pos: [7.25, 0, 0.005], rot: [0, 0, 0]},
      {w: 28, h: 28, pos: [0, 0, -26], rot: [0, 0, 0]},
      {w: 26, h: 28, pos: [-14, 0, -13], rot: [0, Math.PI / 2, 0]},
      {w: 26, h: 28, pos: [14, 0, -13], rot: [0, -Math.PI / 2, 0]},
      {w: 28, h: 26, pos: [0, 14, -13], rot: [Math.PI / 2, 0, 0]},
      {w: 28, h: 26, pos: [0, -14, -13], rot: [-Math.PI / 2, 0, 0]},
    ]

    const geometries = planes.map((p) => {
      ...
      m.compose(new THREE.Vector3(p.pos[0], p.pos[1], p.pos[2]), q, new THREE.Vector3(1, 1, 1))
      geo.applyMatrix4(m)
      return geo
    })
    ...
    const merged = THREE.BufferGeometryUtils
      ...
      : mergeBufferGeometries(THREE, geometries)
...
// Merged portal frame – 4 boxes → 1 draw call
registerComponent('merged-portal-frame', {
  init() {
    const {THREE} = window
    const boxes = [
      {w: 0.01, h: 1.3541666667, d: 0.04, pos: [-0.505, 0, 0.01], color: 0x6c7fb0},
      {w: 0.01, h: 1.3541666667, d: 0.04, pos: [0.505, 0, 0.01], color: 0x6c7fb0},
      {w: 1.02, h: 0.01, d: 0.04, pos: [0, 0.6721, 0.01], color: 0x6c7fb0},
      {w: 1.02, h: 0.01, d: 0.04, pos: [0, -0.6721, 0.01], color: 0x44506c},
    ]
...
// Polyfill for mergeBufferGeometries if BufferGeometryUtils is not available
function mergeBufferGeometries(THREE, geometries) {
  ...
  geometries.forEach((geo) => {
    ...
    for (const key in geo.attributes) {
      ...
      attributes[key].push(geo.attributes[key])
    }
    indexOffset += geo.attributes.position.count
  })

  for (const key in attributes) {
    const arrays = attributes[key]
    const totalLen = arrays.reduce((sum, a) => sum + a.array.length, 0)
    const mergedArray = new Float32Array(totalLen)
    let offset = 0
    arrays.forEach((a) => {
      mergedArray.set(a.array, offset)
      offset += a.array.length
    })
    merged.setAttribute(key, new THREE.BufferAttribute(mergedArray, arrays[0].itemSize))
  }
